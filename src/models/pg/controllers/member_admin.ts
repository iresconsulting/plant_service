...
namespace MemberAdmin {
  export async function create(
    ...
    const sql = `
      INSERT INTO member_admin(no, name, username, password, roles)
      VALUES($1, $2, $3, $4, $5)
      RETURNING *
    `
  ...
  export async function getAll(payload?: any): Promise<Array<any> | false> {
    let sql = `
      SELECT id, no, username, name, roles, access_token, refresh_token, account_status, last_login, created_at, last_updated
      FROM member_admin
  ...
  export async function getPasswordByUsername(no: string): Promise<Array<any> | false> {
    const sql = `
      SELECT password
      FROM member_admin
      WHERE username = $1
  ...
  export async function getByUsernameAndPwd(username: string, pwdPlainText: string): Promise<Array<any> | false> {
    const getUser = await getPasswordByUsername(username)    
    if (!getUser || !getUser.length) {
      return false
    }
    const user = getUser[0]
